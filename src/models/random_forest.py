...
Outputs:
  ...
  artifacts/forecasts/random_forest_test.csv
...
joblib.dump(best_model, ARTM / "random_forest.pkl")

pred = best_model.predict(X_test)
pd.Series(pred, index=X_test.index).to_csv(ARTF / "random_forest_test.csv")
...
row = pd.DataFrame({"model": ["RandomForest"], "MAE": [mae], "RMSE": [rmse], "MAPE": [mape]})
...
print("Saved model →", ARTM / "random_forest.pkl")
print("Saved forecast →", ARTF / "random_forest_test.csv")
